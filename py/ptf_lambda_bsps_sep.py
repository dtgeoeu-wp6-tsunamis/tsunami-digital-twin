...
def compute_ps_bs_gaussians_general(**kwargs):
    ...
    #lambda_mix = True  # True if lambda = ]1,0[
...
# 
#     # Issue #52. It seems that case len(bs_idx) = len(ps_idx) may occour and
#     # else statement is not correct for such case, so add if and lambda_ps = lambda_ps = 0.5
#     lambda_mix = False
#     if(inmoho == True and len(ps_idx) == 0):
#         lambda_ps = 0.0
#         lambda_bs = 1.0
#     elif(inmoho == True and len(set(ps_idx)) == len(set(bs_idx))):
#         lambda_ps = 0.5
#         lambda_bs = 0.5
#         lambda_mix = True
...
# 
    ...
    # lambda_bsps['lambda_mix']  = lambda_mix
