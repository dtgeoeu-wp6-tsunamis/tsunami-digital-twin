...
def parse_line():
    ...
    parser.add_argument("--failed_SBS", default = None,
                                      help = "Filename where failed SBS simulations are listed. Default: None")
    ...
    parser.add_argument("--outfile_SBS", default = None,
                                      help = "Output filename for SBS. Default: None")
    ...
    parser.add_argument("--file_simdir_SBS", default = "",
                                help = "Input SBS folder containing nc files. Default: None")
...
if(args.file_simdir_SBS is not None):
    sbs_path = args.file_simdir_SBS
else:
    sys.exit("SBS file path is missing")
...
if(args.failed_SBS is not None):
    failed_sbs  = args.failed_SBS

if(args.outfile_BS is not None):
    outfile_bs  = args.outfile_BS
if(args.outfile_PS is not None):
    outfile_ps  = args.outfile_PS
if(args.outfile_SBS is not None):
    outfile_sbs  = args.outfile_SBS
...
if os.path.exists(sbs_path):
    with open(sbs_path) as f:
        scenarios_sbs = [line.rstrip() for line in f]
    save_ts_out_ptf_tot(scenarios_sbs, failed_sbs, outfile_sbs, poi_depth, outdir)
