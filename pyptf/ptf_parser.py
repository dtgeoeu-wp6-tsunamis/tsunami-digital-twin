...
#                  'Example for rabbit connetion mode:\n'  + sys.argv[0] + ' --cfg ../cfg/ptf_main.config --mode rabbit'
    ...
    parser.add_argument('--mag_sigma_val',     default = '0.2',      help = 'Assumed magnitude event sigma. Needs --mag_sigma_fix=Yes. Default=0.2')
    parser.add_argument('--sigma',             default = '2.0',       help = 'real numbers indicating the proportion of standard deviation surrounded by each ellipse. ' +\
    ...
    parser.add_argument('--logn_sigma',        default = '1.',      help = 'Sigma value for the lognormal distribution applied to the scenarios. Default=1')
...
def check_arguments(**kwargs):
    ...
    if args.cfg is None:
        print(f"Please provide the correct path of the configuration file: {args.cfg}")
        sys.exit()

    if args.input_workflow is None:
        print(f"Please provide the correct path of the input workflow_input.json: {args.input_workflow}")
        sys.exit()
    ...
    if(args.mode == 'event' and args.event_file == None):
        print('Please provide an event file (use --event_file)')
        sys.exit()
