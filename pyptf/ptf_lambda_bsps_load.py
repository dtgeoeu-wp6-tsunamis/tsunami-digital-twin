...
def get_gaussian_ellipsoid_tetraedons(**kwargs):
    ...
    logger.info(" --> N. Tetra in the Gaussian Ellipsoid: {0}".format(n_tetra))
    ...
    logger.info(" --> Volume of Tetra in the Gaussian Ellipsoid: %.8e [m^3]" % volume_tot)

    Vol_diff_perc = (el['gaussian_ellipsoid']['vol'] - volume_tot) / el['gaussian_ellipsoid']['vol']*100
    logger.info(" --> Volume difference Gaussian <--> Tetra: %.2f [%%]" % Vol_diff_perc)
...
def get_gaussian_ellipsoid_3d(**kwargs):
    ...
    if np.any(w < 0):
        logger.info('Warning: negative eigenvalues')
        w = max(w,0)
    ...
    logger.info(" --> Volume of the Gaussian Ellipsoid: {:.8e} [m^3]".format(volume))
