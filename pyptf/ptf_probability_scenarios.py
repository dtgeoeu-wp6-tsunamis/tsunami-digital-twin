...
def compute_probability_scenarios(**kwargs):
    ...
    if (short_term['BS_computed_YN'] == True and pre_selection['BS_scenarios'] == True):
        ...
        probability_scenarios = bs_probability_scenarios(bs_empty_regions = workflow_dict['bs_empty_regions'],
                                                         logger           = logger,
        ...
        logger.info(' --> Used regions for BS : {}'.format(probability_scenarios['relevant_scenarios_bs']))
    ...
    logger.info(' --> Total_BS_Scenarios: {:7d}'.format(probability_scenarios['nr_bs_scenarios']))
    logger.info(' --> Total_PS_Scenarios: {:7d}'.format(probability_scenarios['nr_ps_scenarios']))
 
    if (probability_scenarios['nr_bs_scenarios'] != short_term['Total_BS_Scenarios']):
        check_bs = 'WARNING'
        logger.info(' --> Check Nr BS scenarios: {:7d} <--> {} --> {}'.format(probability_scenarios['nr_bs_scenarios'], short_term['Total_BS_Scenarios'], check_bs))
    if (probability_scenarios['nr_ps_scenarios'] != short_term['Total_PS_Scenarios']):
        check_ps = 'WARNING'
        logger.info(' --> Check Nr PS scenarios: {:7d} <--> {} --> {}'.format(probability_scenarios['nr_ps_scenarios'], short_term['Total_PS_Scenarios'], check_ps))
...
def bs_probability_scenarios(**kwargs):
    ...
    empty_scenarios = kwargs.get('bs_empty_regions', None)
    logger          = kwargs.get('logger', None)
    short_term      = kwargs.get('short_term', None)
    ...
    for i1 in range(sel_mag):
        ...
        for i2 in range(bs2_pos):
            ...
            if (ireg not in regions_nr):
                ...
                # logger.info("...............................", ireg, region_info, region_files)

            RegMeanProb_BS4 = region_info[ireg]['BS4_FocMech_MeanProb_valNorm']
            if(RegMeanProb_BS4.size == 0):
                 logger.info(' --> WARNING: region info %d is empty!!!' % (ireg) )
...
def set_if_compute_scenarios(**kwargs):
    ...
    logger.info(short_term['magnitude_probability'])
    logger.info(short_term['RatioBSonTot'])
    logger.info(short_term['RatioPSonTot'])

    logger.info(' --> Negligible Probability: {:.4f}'.format(negl_prob))
    logger.info(' --> Sum of probability BS scenarios: {:%.4f}  --> Compute Probability Scenarios: %r'.format(tmpbs, BScomputedYN))
    logger.info(' --> Sum of probability PS scenarios: {:%.4f}  --> Compute Probability Scenarios: %r'.format(tmpps, PScomputedYN))
